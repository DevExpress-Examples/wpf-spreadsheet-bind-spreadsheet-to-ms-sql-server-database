...
Namespace WpfSpreadsheet_BindToDataSource
	...
	Partial Public Class MainWindow
		...
		Private Sub spreadsheetControl_PreviewMouseLeftButtonDown(ByVal sender As Object, ByVal e As System.Windows.Input.MouseButtonEventArgs)
			...
			Dim point As New System.Drawing.Point(CInt(Math.Truncate(winPoint.X)), CInt(Math.Truncate(winPoint.Y)))
		...
		Private Sub AddRow(ByVal sheet As Worksheet)
			...
			Catch ex As Exception
				Dim message As String = String.Format("Cannot add a row to a database table." & vbLf & "{0}", ex.Message)
		...
		Private Sub ApplyChanges()
			...
			Catch ex As Exception
				Dim message As String = String.Format("Cannot update data in a database table." & vbLf & "{0}", ex.Message)
				MessageBox.Show(message, "Error", MessageBoxButton.OK, MessageBoxImage.Error)
			End Try
		End Sub
